...
export JobProjectRootDir=~$HOME/RiderProjects/SecureVerticalSlice/JobProject
#openssl genrsa -aes-256-cbc -out rootCA.key 4096

cd $JobProjectRootDir/dev-cert/dev
...
cp rootCA.key $HOME:/vault/cert
cp rootCA.crt $HOME:/vault/cert

export OPENSSL_PASSWORD='paramount'
cd $JobProjectRootDir/certs/dev/vault
...
cp ./server.vault.key $HOME:/vault/cert
cp ./server.vault.csr $HOME:/vault/cert
cp ./server.vault.crt $HOME:/vault/cert

export OPENSSL_PASSWORD='paramount'
cd $JobProjectRootDir/certs/dev/consul
...
export OPENSSL_PASSWORD='paramount'
cd $JobProjectRootDir/certs/dev/keycloak
